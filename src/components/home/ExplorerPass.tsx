...
import { useTranslation, INTEREST_LABEL_KEYS, LANGUAGE_LABEL_KEYS } from "@/lib/i18n/dictionary";
import type { DictionaryKey } from "@/lib/i18n/dictionary";

const INTERESTS = ["Wine", "Mountains", "History", "Food", "Adventure"];
const LANGUAGES = ["English", "Russian"];

const BENEFITS: { icon: typeof Zap; labelKey: DictionaryKey }[] = [
  { icon: Zap, labelKey: "explorerPass.benefitSaveXp" },
  { icon: Award, labelKey: "explorerPass.benefitCollectBadges" },
  { icon: Ticket, labelKey: "explorerPass.benefitTrackBookings" },
  { icon: Gift, labelKey: "explorerPass.benefitUnlockRewards" },
  { icon: MessageCircle, labelKey: "explorerPass.benefitStayConnected" },
];
    ...
    >
      ...
      >
        ...
        >
          {BENEFITS.map(({ icon: Icon, labelKey }) => (
            <div key={labelKey} className="flex items-center gap-2.5">
              ...
              <span className="text-[13px]" style={{ color: "#3D2B1A" }}>{t(labelKey)}</span>
            </div>
          ))}
        </div>
        ...
        <div>
          <label className="block text-[11px] font-semibold uppercase tracking-wider mb-1.5"
            style={{ color: "#7B6F63" }}>
            {t("explorerPass.whatsappLabel")}
          </label>
          <input
            value={whatsappNumber}
            onChange={(e) => setWhatsappNumber(e.target.value)}
            placeholder={t("explorerPass.whatsappPlaceholder")}
            type="tel"
            inputMode="tel"
            className="w-full px-4 py-3 rounded-xl text-sm focus:outline-none transition-colors"
            style={{
        ...
        <div>
          <label className="block text-[11px] font-semibold uppercase tracking-wider mb-2"
            style={{ color: "#7B6F63" }}>
            {t("explorerPass.supportLanguage")}
          </label>
